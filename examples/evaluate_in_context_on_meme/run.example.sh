...
RUN_ID="meme_${MODE}_in_context_official_02"
CONDA_ENV="membase-meme-incontext"
...
exec "${python_command[@]}" "${repo_root}/scripts/run_with_progress.py" \
  ...
  --benchmark meme --baseline in_context --mode "$MODE" \
  --output-dir "${repo_root}/experiments/meme_in_context/runs" --run-id "$RUN_ID" \
