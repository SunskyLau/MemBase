...
独立 `6k` 模式仅运行两组完整 6k 历史与各 100 题。OurMem 的 `run_6k.sh` 当前使用新运行名 `mab_6k_ourmem_04`、并发 2，从 `envs/.env` 读取凭据。本轮只准备入口，不自动运行正式实验。
...
仅修复读取代码时，已完成构建的静态问答运行可显式复用原记忆：先停止进程，在 `membase-ourmem` 环境执行 `python scripts/prepare_read_resume.py --run-dir <运行目录>` 检查；确认后加 `--clear-read-results`，将旧检索、回答和评分移入 `read_revisions/<修订号>/discarded/`。原数据库、构建快照、配置、请求账本及费用记录保持不变；读取修订（read revision）另行固定当前源码，后续只允许检索和评测。构建相关代码、数据或方法配置改变时拒绝复用；不适用于 MEME 的在线观察点。

当前 `mab_6k_ourmem_4o_mini_01` 修订准备后使用 `evaluate_ourmem_on_memoryagentbench/resume_6k_4o_mini.sh`，支持 `--dry-run` 和任意工作目录启动。它读取原运行配置与 `envs/.env`，依次执行检索、回答和评分，不再构建，也不会重复清理新结果。既有未决输入与维护警告仍保留；控制台不重播修订前的错误，但累计请求数及失败数不会清零。
...
OurMem 的内容纠错最多额外两次，网络恢复独立采用默认 900 秒窗口。窗口耗尽后保存检查点并暂停。抽取或协调的局部语义未决保留原文和待办，其他独立事实继续处理；已确认失去全部支持的结论继续暂停。新提案被拒绝或可选归纳预算用完是正常探索结果，不自动记成维护缺口。相同已处理条件不因刷新再次生成，不新增独立后台任务服务。
...
局部语义未决不等于底层存储故障。网络恢复窗口耗尽、索引或必要删除的技术故障仍暂停相关工作。凭据无效、预算耗尽、产物损坏、源码不一致和内部程序错误不会被计零掩盖；保留已有产物，返回非零码，并在 `partial_summary.json` 列出覆盖情况，不输出完整主成绩。判断本次是否完成还应检查样本级状态，不能只看总进程是否存活。纯运行配置保存于 `execution.json`，方法配置仍受一致性检查保护。

进度分别展示输入未决、实际待修复目标、可选探索结束以及接口/校验失败；不能将它们相加解释为坏记忆数量。校验失败保留真实请求用量，不重复计费。新数据库格式为 `ourmem-v5-3`；旧运行原样保留，不自动迁移。
