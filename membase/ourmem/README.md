...
网络故障与内容纠错分开计数。OurMem 的恢复窗口默认 900 秒，退避 5、15、30、60、120 秒，之后每次 120 秒；单请求超时默认 300 秒，并受剩余恢复窗口限制。窗口耗尽后保存检查点并暂停。结构纠错最多额外两次，所有真实请求、失败和重试进入账本；SDK 不再自行重试。公共调用器的 A-MEM 单请求超时同为 300 秒，额外重试次数仍为两次；官方原生基线的独立调用实现不随此默认值改变。
